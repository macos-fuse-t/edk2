  ...
  )
{
  ...
  // Remap the frambuffer and mark it non-executable
  Status = gCpu->SetMemoryAttributes (
                  gCpu, Private->FbAddr,
                  ALIGN_VALUE (Private->FbSize, EFI_PAGE_SIZE),
                  EFI_MEMORY_WC | EFI_MEMORY_XP);
  if (EFI_ERROR(Status)) {
    DEBUG((
      DEBUG_ERROR, 
      "Failed to set framebuffer memory attributes\n"
    ));
    goto Done;
  }
...
{
  EFI_STATUS  Status;
DEBUG((DEBUG_INFO, "InitializeEmuGop\n"));
