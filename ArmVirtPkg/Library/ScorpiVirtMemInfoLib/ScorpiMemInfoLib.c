...
/**
  ...
  ScorpiMemNode which will be consumed by ArmVirtGetMemoryMap.
...
RETURN_STATUS
EFIAPI
ScorpiMemInfoPeiLibConstructor (
  VOID
  )
{
  VOID                         *DeviceTreeBase;
  ...
  ZeroMem (ScorpiMemNode, sizeof (ScorpiMemNode));
  ...
  for (Prev = 0; ; Prev = Node) {
    ...
    if ((Type != 0) && (AsciiStrnCmp (Type, "memory", Len) == 0)) {
      ...
      if ((RegProp != 0) && (Len == (2 * sizeof (UINT64)))) {
        ...
        ScorpiMemNode[Index].Base = CurBase;
        ScorpiMemNode[Index].Size = CurSize;
        Index++;

        if (Index >= SCORPI_MAX_MEM_NODE_NUM) {
          DEBUG ((
            DEBUG_WARN,
            "%a: memory node larger than %d will not be included into Memory System\n",
            __func__,
            SCORPI_MAX_MEM_NODE_NUM
            ));
          break;
        }
      } else {
  ...
  )
{
  ...
  MemNodeIndex = 0;
  // System DRAM
  while ((MemNodeIndex < SCORPI_MAX_MEM_NODE_NUM) && (ScorpiMemNode[MemNodeIndex].Size != 0)) {
    VirtualMemoryTable[Index].PhysicalBase = ScorpiMemNode[MemNodeIndex].Base;
    VirtualMemoryTable[Index].VirtualBase  = ScorpiMemNode[MemNodeIndex].Base;
    VirtualMemoryTable[Index].Length       = ScorpiMemNode[MemNodeIndex].Size;
  ...
  VirtualMemoryTable[Index].VirtualBase  = VirtualMemoryTable[Index].PhysicalBase;
  VirtualMemoryTable[Index].Length       = SCORPI_GIC_SIZE;
  VirtualMemoryTable[Index].Attributes   = ARM_MEMORY_REGION_ATTRIBUTE_DEVICE;
  Index++;
